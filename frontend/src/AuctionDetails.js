...
export default function AuctionDetails({ isAuthenticated }) {
  ...
  const isAuctionClosed = () => {
    ...
    try {
      return new Date() > new Date(auction.ends);
    } catch {
      return false;
    }
  };

  // Get current user from localStorage (assumes username is stored after login)
  const currentUser = localStorage.getItem("username");
  const isOwnAuction = auction && currentUser && auction.sellerUserId === currentUser;
  ...
  return (
    <div className="container py-5">
      ...
      <div className="card shadow-sm mb-4 position-relative">
        ...
        <div className="card-body" style={{paddingTop: isAuctionClosed() ? '2.5rem' : undefined}}>
          ...
          <div>Current Price: <b>{getCurrentPrice()}</b></div>
      ...
      {bids.length === 0 ? (
        ...
        <ul className="list-group mb-4">
          {bids.map((bid) => (
            <li className="list-group-item d-flex justify-content-between align-items-center" key={bid.id}>
              <span>Bidder: {bid.bidderUsername}</span>
              <span>Amount: <b>{bid.amount}</b></span>
      ...
      {isAuthenticated && !isAuctionClosed() && !isOwnAuction ? (
        <div className="mb-4">
          <h5>Submit a Bid</h5>
          <form onSubmit={handleBidSubmit} className="row g-2 align-items-center">
            ...
            <div className="col-auto">
              <button type="submit" className="btn btn-primary" disabled={submitting || !bidAmount}>
                Submit Bid
              </button>
            </div>
          </form>
          {showConfirm && (
            <div className="alert alert-warning mt-3">
              <div>Are you sure you want to submit this bid? This action cannot be undone.</div>
              <button className="btn btn-success me-2 mt-2" onClick={confirmBid} disabled={submitting}>Yes, Submit</button>
              <button className="btn btn-secondary mt-2" onClick={() => setShowConfirm(false)} disabled={submitting}>Cancel</button>
            </div>
          )}
        </div>
      ) : isAuctionClosed() ? (
        <div className="alert alert-secondary">Bidding is closed for this auction.</div>
      ) : isOwnAuction ? (
        <div className="alert alert-warning">You cannot bid on your own auction.</div>
      ) : (
        <div className="alert alert-info">You must be logged in to submit a bid. Please log in or register.</div>
      )}
    </div>
  );
}
